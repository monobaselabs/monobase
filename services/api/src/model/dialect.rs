...
impl Dialect {
    ...
    /// PostgreSQL needs `::jsonb`; SQLite stores JSON as TEXT (no cast).
    ...
    /// SQLite: falls back to `LIKE` (case-insensitive by default).
    ...
    /// PostgreSQL uses the pattern as-is; SQLite wraps it in `%pattern%` for LIKE.
    ...
    /// e.g., checking if a JSONB array contains an element matching `{"uid":"a"}`.
    ///
    /// PostgreSQL: `column @> '[{"uid":"a"}]'::jsonb`
    /// SQLite: `EXISTS (SELECT 1 FROM json_each(column) WHERE json_extract(value, '$.uid') = 'a')`
    ...
    /// e.g., checking if a JSONB array `["a","b"]` overlaps with `["a"]`.
    ///
    /// PostgreSQL: `column ?| array['a']::text[]`
    /// SQLite: `EXISTS (SELECT 1 FROM json_each(column) WHERE value IN ('a'))`
    ...
    ///
    /// PostgreSQL: `column->>'key'` or `column->'a'->>'b'`
    /// SQLite: `json_extract(column, '$.key')` or `json_extract(column, '$.a.b')`
...
mod tests {
    ...
    #[test]
    fn test_regex() {
        assert_eq!(Dialect::Postgres.regex_op(true), "~*");
        assert_eq!(Dialect::Postgres.regex_op(false), "~");
        assert_eq!(Dialect::Sqlite.regex_op(true), "LIKE");
        assert_eq!(Dialect::Postgres.regex_pattern("John"), "John");
        assert_eq!(Dialect::Sqlite.regex_pattern("John"), "%John%");
    }
    ...
    fn test_json_extract() {
        ...
        assert_eq!(
            Dialect::Postgres.json_extract_text("_data", &["config", "email"]),
            "\"_data\"->'config' ->>'email' "
        );
        assert_eq!(
            Dialect::Sqlite.json_extract_text("_data", &["config", "email"]),
            "json_extract(\"_data\", '$.config.email')"
        );
    }
}
